...
lod.oneplot <- function (input, yrange = NULL, xlab.text="chromosome",
                      mar.set =c(4.5, 4.5, 0.5, 0.5), mgp.set = c(2.5, 0.8, 0),
                      cols = c("grey16", "grey32"), lod.cutoff = NULL,
  ...
  # input should have three columns, "Chr", "Pos", "lod"
  ...
  # lod cutoff
  if (!is.null(lod.cutoff)) {
	abline(h = lod.cutoff, col = "red", lwd = cex.line.width, lty = 2) #darkorange2
  }
